...
test("CJS native runtime: acknowledged LMDB commit survives process exit without close", async () => {
  ...
  try {
    const script = `
      ...
      const { openKeyValueStorage } = require(${JSON.stringify(nodePath)});
      (async () => {
        const items = hydb.table("items", { id: id().primaryKey(), value: text().notNull() });
        const storage = await openKeyValueStorage({ schema: hydb.schema({ items }), directory: process.argv[1] });

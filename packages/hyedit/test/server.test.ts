...
import { createHyeditClient } from "../src/client.js";
import { createHyeditServer } from "../src/server.js";

test("serves the bootstrap script and iframe document", async () => {
  const telemetryDirectory = mkdtempSync(join(tmpdir(), "hyedit-test-"));
  ...
  try {
    ...
    assert.match(clientScript, /hyedit-launcher/);

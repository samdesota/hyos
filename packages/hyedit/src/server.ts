...
import { loadNearestHyeditEnvironment } from "./environment.js";
import {
...
import { createHyeditRouter } from "./trpc.js";
import {
...
export interface HyeditServerOptions {
  host?: string;
  port?: number;
  projectRoot?: string;
  apiKey?: string;
  model?: string;
  models?: HyeditModelOption[];
...
export function createHyeditServer(
  options: HyeditServerOptions = {},
): HyeditServer {
  const host = options.host ?? "127.0.0.1";
  const port = options.port ?? 4317;
  const projectRoot = options.projectRoot ?? process.cwd();
  const environment = loadNearestHyeditEnvironment(projectRoot);
  let serverUrl: string | undefined;
  const telemetry = createDevelopmentTelemetry(projectRoot, options.telemetry);
  const defaultModel =
    options.model ??
    process.env.HYEDIT_MODEL ??
    environment.HYEDIT_MODEL ??
    DEFAULT_HYEDIT_MODEL;
    ...
    (apiKey
      ? createQuickIterationAgent({
          ...
          reasoning: parseGatewayReasoningEffort(
            options.reasoning ??
              process.env.HYEDIT_REASONING ??
              environment.HYEDIT_REASONING,
          ),
          providerOrder:
            options.providerOrder ??
            (
              process.env.HYEDIT_PROVIDER_ORDER ??
              environment.HYEDIT_PROVIDER_ORDER
            )
              ?.split(",")
      ...
      : unavailableAgent);
  const router = createHyeditRouter(agent, undefined, telemetry, {
    defaultModel,
    models,
  });

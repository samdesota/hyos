...
const MODEL_KEY = "hyedit-model";

function post(message: OverlayMessagePayload): void {
  window.parent.postMessage({ source: "hyedit", ...message }, "*");
}
...
function App() {
  ...
  useEffect(() => {
    const onMessage = (event: MessageEvent<unknown>) => {
      ...
      if (message?.source !== "hyedit-host") return;
  ...
  useEffect(() => {
    void getHyeditConfiguration().then((configuration) => {
...
export function mountHyeditOverlay(root: HTMLElement): () => void {
...
if (root) mountHyeditOverlay(root);

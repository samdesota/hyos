...
const FRAME_ID = HYEDIT_FRAME_ID;
const LAUNCHER_ID = HYEDIT_LAUNCHER_ID;
const SOURCE_ATTRIBUTE = "data-source-loc";
const SESSION_KEY = "hyedit-iteration";
...
function postToOverlay(message: HostMessagePayload): void {
  const payload = { source: "hyedit-host", ...message };
  const overlay = frame();
...
function mountOverlay(): void {
  if (!frame()) {
    ...
    if (overlay instanceof HTMLIFrameElement) {
      ...
      root.className = "hyedit-root";
      shadow.append(stylesheet, root);
      const moduleUrl = new URL("./overlay.js", scriptUrl).href;
      void import(moduleUrl)
        .then(
          (module: { mountHyeditOverlay(root: HTMLElement): () => void }) => {
            if (overlay.isConnected)
              unmountOverlay = module.mountHyeditOverlay(root);
          },
        )
...
const handleMessage = (event: MessageEvent<unknown>): void => {
  ...
  if (message.source !== "hyedit") return;
...
const disposeHyeditHost = (): void => {
  window.clearInterval(flushTimer);
  window.removeEventListener("error", handleWindowError);
  window.removeEventListener("unhandledrejection", handleUnhandledRejection);
  window.removeEventListener("keydown", handleKeydown);
  window.removeEventListener("message", handleMessage);
  window.removeEventListener(HYEDIT_DISPOSE_EVENT, disposeHyeditHost);
...
window.addEventListener(HYEDIT_DISPOSE_EVENT, disposeHyeditHost);

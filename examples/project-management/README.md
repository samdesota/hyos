...
handlers. The server retains the authoritative handlers and binds every request
to a principal before it reaches the gateway.
...
The user selector is deliberately simple demo authentication. It sends a
seeded user ID to the backend; the backend validates that identity and supplies
the resulting principal context to the gateway. Authorization is not performed
in the UI:
...
live updates, reload persistence, and principal switching.
